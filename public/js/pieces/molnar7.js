var colorPalettes;
var currentPalette;
var t = 0;  // time variable for color transitions

colorPalettes = [
  ['#8B624B', '#D57254', '#E79C33', '#E27B98'],
  ['#E48504', '#F0C73C', '#B41840', '#8D847D'],
...
function draw() {
  ...
  createSandTexture();
  blendMode(ADD);  // set blending mode for layering
  strokeWeight(1.420);
  var nCellCols = 5;
  ...
  for (var j = 0; j < nCellRows; j++) {
    for (var i = 0; i < nCellCols; i++) {
      strokeWeight(2.5 + 0.1 * (i + j));  // variable stroke weight

      // interpolate between two colors in the palette
      var color1 = color(currentPalette[floor(random(currentPalette.length))]);
      var color2 = color(currentPalette[floor(random(currentPalette.length))]);
      var lerpedColor = lerpColor(color1, color2, sin(t));
      stroke(lerpedColor);
  ...
  if (t >= TWO_PI) {  // reset after a full cycle
    t = 0;
    currentPalette = colorPalettes[floor(random(colorPalettes.length))];  // change color palette
  }
}
